...
// Service : 비즈니스 로직 처리 
// - DB에 CRUD 후 결과 반환 받기
//   + DML 성공여부에 따른 트랜잭션 제어 처리(commit/rollback)  
//	   --> commit/rollback에는 Connection 객체가 필요하기 때문에
//		 Connection 객체를 Service에서 생성후
...
public class UserService {
	...
	public int insertUser(User user) throws Exception {
		...
		if(result > 0) { // INSERT 성공
			commit(conn);
		} else { // INSERT 실패 (사실 왠만하면 실패할리가 없긴함...)
			rollback(conn);
		}
	...
	public int deleteUser(int input) throws Exception{
		...
		// 1. 데이터베이스의 정보를 가지고 있는 객체 이게 있어야 정보들이 ㅇㅇ
		Connection conn = getConnection();
		
		// 2. 데이터 가공(없으면 패쑤)
		
		// 3. DAO 메서드(DELETE) 호출후 결과(삭제된 행의 개수, int) 반환 받기
